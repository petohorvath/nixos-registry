...
let
  ...
  checkRoot =
    ...
    if evaluation._module.freeformType != null then
      throw (
        "nixos-registry: unrestricted freeform roots are unsupported; "
        + "declare options in schemaModules."
      )
    else
      evaluation;
  ...
  module = {
    options.registry =
      lib.mkOption {
        ...
        default = { };
        description = ''
          This node's contribution to the shared registry.
        '';
      }
      // {
    ...
    builtins.addErrorContext "while collecting registry data from node `${name}':" (
      ...
      if !(node ? options.registry) then
        throw (
          ...
          + "import registry.module or configure "
          + "the static nixosModules.default."
        )
      else if
        !(lib.isOption option)
      ...
      then
        throw (
          "nixos-registry: node `${name}` "
          + "has an incompatible options.registry; "
          + "import registry.module or configure "
          + "the static nixosModules.default."
        )
      else
