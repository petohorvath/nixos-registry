...
Run commands from the repository root with Nix's `nix-command` and `flakes` features enabled. The [root flake](../flake.nix) supplies the dependencies pinned in [flake.lock](../flake.lock). Commands using `.stable` also accept `.unstable` to select the other tested Nixpkgs revision.
...
```sh
nix eval --no-update-lock-file .#lib.nixosExamples.stable --json
nix eval --no-update-lock-file .#lib.nixosExamples.stable.validate
```
...
An optional `package` argument selects the Prometheus package. With the root flake's inputs, the call looks like this:
...
```sh
nix eval --no-update-lock-file .#lib.examples.stable --json
nix eval --no-update-lock-file .#lib.examples.stable.validate
```
...
The composition module also exposes validation through `perSystem.checks`. The root flake evaluates this example with each pinned Nixpkgs revision:

```sh
nix eval --no-update-lock-file .#lib.flakePartsExamples.stable --json
nix eval --no-update-lock-file .#lib.flakePartsExamples.unstable --json
```
...
| ------------------------------------------------------------------------------ | ---------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| [Partial contributions](../examples/plain-nix/partial-contributions.nix)       | `nix eval --no-update-lock-file .#lib.partialContributions.stable --json`    | Completed records from different sources, schema defaults, derived values, and the fields available locally |
| [Override priorities](../examples/plain-nix/priorities.nix)                    | `nix eval --no-update-lock-file .#lib.priorities.stable --json`              | A default contribution, a forced contribution, and an override of one port                                  |
| [Conditions and list ordering](../examples/plain-nix/conditional-ordering.nix) | `nix eval --no-update-lock-file .#lib.conditionalOrdering.stable --json`     | Enabled contributions around the central paths; central paths alone when disabled                           |
| [Combined reads](../examples/plain-nix/combined-reads.nix)                     | `nix eval --no-update-lock-file .#lib.combinedReads.stable --json`           | A contributed host using the shared domain, completed with the central port                                 |
| [Lazy collection](../examples/plain-nix/collection-laziness.nix)               | `nix eval --no-update-lock-file .#lib.collectionLaziness.stable.lazy --json` | Domain and endpoint under a `lazyAttrsOf` option, with successful validation                                |

To demand validation separately, use the attribute exposed by the example:

```sh
nix eval --no-update-lock-file .#lib.partialContributions.stable.validate
nix eval --no-update-lock-file .#lib.priorities.stable.forcedContribution.validate
nix eval --no-update-lock-file .#lib.conditionalOrdering.stable.enabled.validate
```
...
```sh
nix eval --no-update-lock-file .#lib.scalarConflicts.stable
```
...
```sh
nix eval --no-update-lock-file .#lib.collectionLaziness.stable.strict.combined.settings.domain
nix eval --no-update-lock-file .#lib.valueCycles.stable.combined.services.east
```
